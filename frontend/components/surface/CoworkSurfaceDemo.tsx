...
function Composer({
  ...
  const t = UI_TEXT[locale];
  // Follow-ups come directly from the artifact — generated by the LLM,
  // not hardcoded rules. They're specific to what the model actually produced.
  const followUps = useMemo(() => {
    if (!lastArtifact) return [];
    const spec = lastArtifact.schema_json as Record<string, unknown>;
    const fups = spec.follow_ups;
    return Array.isArray(fups) ? (fups as string[]).slice(0, 3) : [];
  }, [lastArtifact]);
