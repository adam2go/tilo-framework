...
from tilo.core.database import Base
...
class SurfaceTurn(Base):
    """One ROAM "Render" iteration emitted by a Run.

    A Run produces zero or more SurfaceTurns in order. Each turn corresponds
    to one focused decision/result rendered by the agent. The turn carries
    both the policy decision that triggered it (for auditability) and the
    fully-validated SurfaceSpec consumed by the renderer.
    """

    __tablename__ = "surface_turns"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    run_id: Mapped[str] = mapped_column(String, ForeignKey("runs.id"), index=True)
    session_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("conversation_sessions.id"), nullable=True, index=True
    )
    workspace_id: Mapped[str] = mapped_column(String, ForeignKey("workspaces.id"), index=True)
    ...
    budget_hint: Mapped[str] = mapped_column(String, default="mini")
    status: Mapped[str] = mapped_column(String, default="ready")  # ready | failed | superseded

    # The validated SurfaceSpec JSON as defined by tilo.surface.v1.
    surface_spec_json: Mapped[dict] = mapped_column(JSON, default=dict)

    # The policy decision that produced this turn (for audit + reflection).
    policy_decision_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)

    # Plan-step linkage for traceability.
    plan_step_index: Mapped[int | None] = mapped_column(Integer, nullable=True)
    plan_step_type: Mapped[str | None] = mapped_column(String, nullable=True)

    # Optional FK to a rich Artifact (for escalate_to_rich).
    artifact_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("artifacts.id"), nullable=True, index=True
    )

    # How the surface was composed: "deterministic" | "llm" | "deterministic_fallback"
    composer_mode: Mapped[str] = mapped_column(String, default="deterministic")
