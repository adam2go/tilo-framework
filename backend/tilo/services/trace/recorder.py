...
from tilo.models import TraceStep
...
class TraceRecorder:
    ...
    def append_streaming_chunk(self, step: TraceStep, chunk: str, max_chars: int = 1200) -> TraceStep:
        """Append a streamed chunk to a `running` step's summary.

        Used when the runtime wants to show progressive content (e.g. a
        reasoning stream from the model) directly on the trace step. The
        text is intentionally NOT routed through the standard sanitizer
        because reasoning content is a first-class API field on modern
        providers and is shown deliberately. Length is bounded by
        `max_chars` so the row never grows unbounded; the ellipsis is
        kept at the *start* so the user always sees the latest tail.
        """
        if not chunk:
            return step
        current = step.summary or ""
        combined = current + chunk
        if len(combined) > max_chars:
            combined = "…" + combined[-(max_chars - 1) :]
        step.summary = combined
        self.db.commit()
        self.db.refresh(step)
        return step
