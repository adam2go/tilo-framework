...
class SalesAction(BaseModel):
    ...
    title: str
    detail: str = ""
    status: Literal["ready", "waiting", "done"] = "ready"
...
class SalesFollowUpLLMData(BaseModel):
    metrics: list[SalesMetric] = Field(min_length=2)
    ...
    @classmethod
    def cap_actions(cls, v: list[SalesAction]) -> list[SalesAction]:
        return v[:6]
...
class CompetitiveAnalysisLLMData(BaseModel):
    summary: str
    ...
    @classmethod
    def cap_rows(cls, v: list[CompetitorRow]) -> list[CompetitorRow]:
        return v[:8]
