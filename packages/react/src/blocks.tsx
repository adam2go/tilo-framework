...
import type { ProgressState, Severity } from "./surface";
...
export function TextBlock({ block }: BlockProps<"text">): JSX.Element {
  return <p className="px-1 text-sm leading-relaxed text-slate-700 whitespace-pre-wrap">{block.data.content}</p>;
}
...
export function EvidenceBlock({ block }: BlockProps<"evidence">): JSX.Element {
  ...
  return (
    <figure className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
      <blockquote className="text-sm italic leading-relaxed text-slate-700">"{excerpt}"</blockquote>
      <figcaption className="mt-1 flex items-center gap-1 text-xs text-slate-500">
        <FileText size={12} />{source_label || source_ref}
      </figcaption>
    </figure>
  );
}
    ...
    return (
      <div className="grid grid-cols-2 gap-2">
        {[data.left, data.right].map((side) => (
          <div key={side.label} className={`rounded-md border px-3 py-2 ${severityClass(side.severity)}`}>
            <div className="text-xs font-medium uppercase tracking-wide text-slate-500">{side.label}</div>
            <div className="mt-1 text-sm font-semibold">{side.value}</div>
          </div>
        ))}
      </div>
    );
          ...
          return (
            <button
              ...
              onClick={async () => {
                ...
                try { await fire(option.action_id, { option_id: option.id, value: option.value }); }
              ...
              className={[
                ...
                isPrimary ? "border-slate-900 bg-slate-900 text-white hover:bg-slate-800" : "border-slate-200 bg-white text-slate-800",
              ].join(" ")}
            >
...
export function FormBlock({ block, fire }: BlockProps<"form">): JSX.Element {
  ...
  const update = (name: string, value: unknown) => setValues((c) => ({ ...c, [name]: value }));
  const canSubmit = !submitting && fields.every((f) => !f.required || (values[f.name] !== "" && values[f.name] != null));
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        if (!canSubmit) return;
        setSubmitting(true);
        try { await fire(submit_action_id, { values }); }
    ...
    >
      {fields.map((field) => (
        <label key={field.name} className="block">
          ...
          {field.kind === "textarea" ? (
            <textarea className="mt-1 w-full rounded border border-slate-300 px-2 py-1 text-sm" placeholder={field.placeholder ?? undefined} required={field.required} value={String(values[field.name] ?? "")} onChange={(e) => update(field.name, e.target.value)} />
          ) : field.kind === "select" ? (
            <select className="mt-1 w-full rounded border border-slate-300 px-2 py-1 text-sm" required={field.required} value={String(values[field.name] ?? "")} onChange={(e) => update(field.name, e.target.value)}>
              <option value="">—</option>
              {field.options.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </select>
          ) : field.kind === "toggle" ? (
            <input type="checkbox" className="mt-1" checked={Boolean(values[field.name])} onChange={(e) => update(field.name, e.target.checked)} />
          ) : (
            <input type={field.kind === "number" ? "number" : field.kind === "date" ? "date" : "text"} className="mt-1 w-full rounded border border-slate-300 px-2 py-1 text-sm" placeholder={field.placeholder ?? undefined} required={field.required} min={field.min ?? undefined} max={field.max ?? undefined} step={field.step ?? undefined} value={String(values[field.name] ?? "")} onChange={(e) => update(field.name, field.kind === "number" ? Number(e.target.value) : e.target.value)} />
          )}
        </label>
      ))}
      <button type="submit" disabled={!canSubmit} className="mt-1 inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50">
...
export function ProgressBlock({ block }: BlockProps<"progress">): JSX.Element {
      ...
      <ul className="space-y-1">
        {data.steps.map((s) => (
          <li key={s.id} className="flex items-center gap-2 text-sm text-slate-700">
            {stepIcon[s.state]}<span className={s.state === "done" ? "text-slate-500 line-through" : ""}>{s.label}</span>
          </li>
        ))}
      </ul>
    );
  ...
  return <div className="flex items-center gap-2 px-1 text-sm text-slate-700"><Loader2 size={14} className="animate-spin" />{data.status}</div>;
}
...
export function LinkBlock({ block }: BlockProps<"link">): JSX.Element {
  ...
  return (
    <a href={url} target={target === "external" ? "_blank" : undefined} rel={target === "external" ? "noreferrer" : undefined} className="inline-flex items-center gap-1 text-sm font-medium text-slate-900 underline-offset-2 hover:underline">
...
export function EditableBlock({ block, fire }: BlockProps<"editable">): JSX.Element {
  ...
  return (
    <div className="rounded-md border border-slate-200 bg-white px-3 py-2">
      <div className="mb-1 flex items-center gap-1 text-xs text-slate-500"><Pencil size={12} />Editable</div>
      <textarea className="w-full rounded border border-slate-300 px-2 py-1 text-sm" rows={Math.min(8, Math.max(3, value.split("\n").length))} value={value} onChange={(e) => setValue(e.target.value)} />
      {block.data.highlights.length ? (
        <ul className="mt-1 space-y-0.5">{block.data.highlights.map((h) => <li key={h} className="text-xs text-slate-500">· {h}</li>)}</ul>
      ) : null}
      <button type="button" disabled={submitting} onClick={async () => { setSubmitting(true); try { await fire(block.data.submit_action_id, { value }); } finally { setSubmitting(false); } }} className="mt-2 inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50">
        {submitting ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}Save
      </button>
    </div>
  );
}
...
export function ArtifactLinkBlock({ block, fire }: BlockProps<"artifact_link">): JSX.Element {
  ...
  return (
    <button type="button" onClick={() => void fire(open_action_id)} className="group flex w-full items-start justify-between gap-3 rounded-md border border-slate-200 bg-white px-3 py-2 text-left transition hover:border-slate-300 hover:bg-slate-50">
...
export function FallbackBlock({ block }: BlockProps<"fallback">): JSX.Element {
  return <p className="rounded-md border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700">{block.data.content}</p>;
}
